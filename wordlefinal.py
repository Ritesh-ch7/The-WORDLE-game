...
hidden=random.choice(word_list)
print(hidden)
word= hidden.lower()

word_list1=[]
for items in word_list:
    word_list1.append(items.lower())

root.config(bg='black')
# Label(root,text='THE WORDLE',font=("comic sans ms",10),bg='white').grid(row=0,column=1,padx=10)

guessnum = 1

wordInput = Entry(root)
wordInput.grid(row=999, column=0, padx=10, pady=10, columnspan=3)
# tryword=wordInput.get()
# if tryword not in word:
#     messagebox.showerror("error","use a valid word")
...
def getGuess():
    ...
    if guessnum <= 6:
        ...
        if len(guess) == 5:
            ...
            if guess not in word_list1:
                messagebox.showerror("OOOPS!", "OOOPS! use a valid word")

            else:

                if word == guess:  # CORRECT
                    messagebox.showinfo("YAYY!", f"YAYY! the word was {word.title()}")

              # INCORRECT
                # if wordInput in word_list:
            ...
            messagebox.showerror("OHHH!", "please use 5 characters in your guess")

    else:
        messagebox.showerror("SORRYYY!", f"You Lose! The word was {word}")
